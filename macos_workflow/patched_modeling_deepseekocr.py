...
# === MPS SUPPORT PATCH (based on HF discussion #20) ===
import os as _dsocr_os
import torch as _dsocr_torch
if _dsocr_torch.backends.mps.is_available():
    _dsocr_os.environ.setdefault("PYTORCH_ENABLE_MPS_FALLBACK", "1")
    _DS_OCR_MPS_IMAGE_DTYPE = _dsocr_os.getenv("DS_OCR_MPS_IMAGE_DTYPE", "bf16").lower()

def _dsocr_mps_rowwise_assign_(target: _dsocr_torch.Tensor, mask: _dsocr_torch.Tensor, src: _dsocr_torch.Tensor):
    if mask is None:
        raise RuntimeError("mask must not be None")
    if mask.dim() == 2 and mask.shape[1] == 1:
        mask = mask.squeeze(1)
    mask = mask.to(_dsocr_torch.bool)
    idx = _dsocr_torch.nonzero(mask, as_tuple=False).squeeze(-1)
    if idx.numel() == 0:
        return target
    n = min(idx.shape[0], src.shape[0])
    idx = idx[:n]
    src = src[:n].to(device=target.device, dtype=target.dtype)
    target.index_copy_(0, idx.to(target.device), src)
    return target

def _dsocr_first_param_dtype(_mod, _default=_dsocr_torch.float32):
    try:
        return next(_p.dtype for _p in _mod.parameters() if _p is not None)
    except StopIteration:
        return _default
# === END MPS SUPPORT PATCH HEADER ===
            ...
            for image, crop_shape in zip(images, images_spatial_crop):
                ...
                # with torch.inference_mode(): 
                    ...
                    if torch.sum(patches).item() != 0:
                        ...
                        crop_flag = 1
                        # Align input dtype with SAM module weights to avoid BF16/FP32 mismatch on MPS
                        try:
                            _sam_dtype = _dsocr_first_param_dtype(sam_model, _dsocr_torch.float32)
                            patches = patches.to(_sam_dtype)
                        except Exception:
                            pass
                        ...
                        try:
                            _sam_dtype = _dsocr_first_param_dtype(sam_model, _dsocr_torch.float32)
                            image_ori = image_ori.to(_sam_dtype)
                        except Exception:
                            pass
                ...
                if images_in_this_batch:
                    ...
                    # --- MPS-safe scatter replacement ---
                    try:
                        inputs_embeds[idx].masked_scatter_(images_seq_mask[idx].unsqueeze(-1).to(images_in_this_batch.device), images_in_this_batch)
                    except Exception as _dsocr_e:
                        _mask = images_seq_mask[idx]
                        inputs_embeds[idx] = _dsocr_mps_rowwise_assign_(inputs_embeds[idx], _mask, images_in_this_batch)
                    # --- END MPS-safe scatter replacement ---
